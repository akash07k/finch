...
| [Logger](./logger.md)                 | Anyone using the logger        | `@finch/logger` core: dispatch, child loggers, transports (Console / IndexedDB / WebSocket)   |
| [Log server](./log-server.md)         | Tool operators                 | `@finch/log-server` CLI, WebSocket and HTTP server, session storage, accessible React viewer  |
