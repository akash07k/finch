...
- Tab navigation uses the WAI-ARIA Tabs keyboard model (Tab into the list, then Left/Right/Home/End). `hotkeys-js` covers two local shortcuts (`Alt+T` cycle theme, `Shift+?` help) plus the global `Alt+M` (mute) and `Alt+Shift+R` (open options) registered with `browser.commands`.
