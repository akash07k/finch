...
`@finch/log-server` is a development-only Node CLI that receives log entries from the extension over WebSocket, persists them per session, and serves a small accessible React UI over HTTP.
...
1. The Finch extension's `WebSocketTransport` sends JSON log entries to the log server.
