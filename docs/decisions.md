...
`Alt+Shift+M` is the global shortcut for toggling the new flag; the listener in `extension/entrypoints/background.ts` reads the current value, flips it, persists, and shows a basic browser notification with the new state. While we were touching the commands block, the long-standing `Alt+Shift+O` for opening the options page moved — first to `Alt+Shift+B`, then to `Alt+Shift+R`, then to `Alt+Shift+I`, and finally to `Alt+Shift+C`. The journey through the alphabet is driven by collisions: `Alt+Shift+F` opens the File menu on Firefox, `Alt+Shift+I` opens the Chrome feedback flow, and `Alt+Shift+N` opens a new Tor Window in Brave (Chromium-based). `C` is the survivor — no known collision on Chrome, Firefox, or Brave on any platform we test.
