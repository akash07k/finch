...
`Alt+Shift+M` is the global shortcut for toggling the new flag; the listener in `extension/entrypoints/background.ts` reads the current value, flips it, persists, and shows a basic browser notification with the new state. While we were touching the commands block, the long-standing `Alt+Shift+O` for opening the options page moved — first to `Alt+Shift+B`, then to `Alt+Shift+R`, then to `Alt+Shift+I`, and finally to `Alt+Shift+C`. The journey through the alphabet is driven by collisions: `Alt+Shift+F` opens the File menu on Firefox, `Alt+Shift+I` opens the Chrome feedback flow, and `Alt+Shift+C` and `Alt+Shift+N` are both silently rejected by Chrome's auto-registration for `suggested_key` (they work when set manually, but new installs get "not set" — unacceptable for the primary screen-reader audience). Dropped Shift entirely and landed on `Alt+I` — simpler keystroke, auto-registers on Chrome, and `Alt+Shift+I` (Chrome feedback) is a different combo.
