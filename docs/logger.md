...
`@finch/logger` is the structured logger used by the extension. It is a small, dependency-free package built around three primitives: a `Logger` interface, a `Transport` interface, and a default `LogEntry` shape.
...
Persists entries to `finch-logs` (database name from `CONFIG.logger.idbName`). Rotation: when the entry count exceeds `CONFIG.logger.idbMaxEntries` (default 10,000), the oldest 10% are deleted. A `rotating` flag prevents concurrent rotate calls from racing each other during burst logging.
...
Forwards entries to a log server (`@finch/log-server`) over WebSocket. The transport is opt-in: the extension only adds it after the user enables log streaming in the options page.
