...
63 browser events across three tiers: 24 essential (Tier 1, 26 enabled by default), 37 useful (Tier 2, opt-in), and 2 advanced (Tier 3). Every event has its own enable toggle, volume slider (0–100%), pitch slider (0.5x–2.0x), and preview button.
...
Keyboard shortcuts: Alt+M toggles mute, Alt+Shift+M toggles mute-when-unfocused, Alt+Shift+I opens options (all global). Inside popup and options: Alt+T cycles themes, Shift+? reads available shortcuts aloud.
