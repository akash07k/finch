 ...
 * `LogLevel.FATAL` from `@finch/logger` (0=DEBUG..4=FATAL).
...
export class LogServer extends EventEmitter {
   ...
   *   allowed. Finch's WebSocketTransport runs from one of these
   *   origins.
  ...
  private handleHttp(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (!this.config.webDir) {
      ...
      res.end("finch-log-server is running. No web viewer built.");
      return;
    }
